...
public class EViewPresenterHolder extends BaseArchitectureComponentHolder {
	...
	public EViewPresenterHolder(EBeanHolder holder) {
		super(holder);
	}
	...
	private void setViewPresenterObserver() {
		...
		JConditional ifViewObserverNeNull = holder().getInitBodyAfterInjectionBlock()._if(getRootViewField().ne(_null()));
		ifViewObserverNeNull._then().invoke(getPresenterLiveDataField(), "observe")
				.arg(cast(getJClass(LIFECYCLE_OWNER), getRootViewField())).arg(presenterLiveDataObserverField);
		ifViewObserverNeNull._else().invoke(getPresenterLiveDataField(), "observeForever").arg(presenterLiveDataObserverField);
